...
**Compact companion: one row per isolated component.** The same contrasts as the lower block above, laid out one component per row with the interval and $k/n$ in their own columns, for the text or the supplement.

```latex
\begin{table*}[t]
\centering
\caption{Contribution of each component of MonoIIF, isolated. Each row is the paired difference in Abs Rel per sequence between the two lines of Table~\ref{tab:components} that differ only in that component (first $-$ second, so a \textbf{negative} value means the component helps), with its 95\% cluster bootstrap interval and, as $k/n$, the number of sequences in which the first is better. $\star$ and bold mark intervals excluding zero. Line~4 is the full method on a randomly initialised encoder, so the first row measures the pretrained backbone with every other component held fixed. SCARED and C3VD have $n=7$ sequences; Hamlyn uses $n=58$ blocks of 100 frames of its single sequence.}
\label{tab:components-isolated}
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{lcccccc}
\toprule
 & \multicolumn{2}{c}{SCARED} & \multicolumn{2}{c}{Hamlyn} & \multicolumn{2}{c}{C3VD} \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}\cmidrule(lr){6-7}
Component isolated & $\Delta$ Abs Rel [95\% CI] & $k/n$ & $\Delta$ Abs Rel [95\% CI] & $k/n$ & $\Delta$ Abs Rel [95\% CI] & $k/n$ \\
\midrule
pretrained weights (5 $-$ 4, everything else equal) & $\mathbf{-0.0549}$$^{\star}$ $[-0.0695, -0.0410]$ & 7/7 & $\mathbf{-0.0840}$$^{\star}$ $[-0.0985, -0.0693]$ & 55/58 & $\mathbf{-0.1372}$$^{\star}$ $[-0.1695, -0.1010]$ & 7/7 \\
DV-LoRA adapters (3 $-$ 2) & $\mathbf{-0.0089}$$^{\star}$ $[-0.0139, -0.0039]$ & 6/7 & $-0.0031$ $[-0.0076, +0.0015]$ & 34/58 & $\mathbf{+0.0323}$$^{\star}$ $[+0.0144, +0.0536]$ & 0/7 \\
DV-LoRA vs plain LoRA (5 $-$ 13) & $+0.0008$ $[-0.0001, +0.0017]$ & 2/7 & $\mathbf{-0.0030}$$^{\star}$ $[-0.0044, -0.0016]$ & 45/58 & $\mathbf{-0.0219}$$^{\star}$ $[-0.0295, -0.0149]$ & 7/7 \\
local calibration alone (6 $-$ 3) & $+0.0000$ $[-0.0018, +0.0022]$ & 3/7 & $\mathbf{+0.0029}$$^{\star}$ $[+0.0014, +0.0045]$ & 18/58 & $\mathbf{-0.0260}$$^{\star}$ $[-0.0360, -0.0160]$ & 7/7 \\
global calibration alone (7 $-$ 3) & $-0.0012$ $[-0.0028, +0.0002]$ & 6/7 & $\mathbf{+0.0032}$$^{\star}$ $[+0.0012, +0.0052]$ & 22/58 & $\mathbf{-0.0256}$$^{\star}$ $[-0.0297, -0.0221]$ & 7/7 \\
illumination-invariant loss (5 $-$ 8) & $+0.0006$ $[-0.0009, +0.0021]$ & 3/7 & $-0.0015$ $[-0.0029, +0.0000]$ & 38/58 & $\mathbf{+0.0170}$$^{\star}$ $[+0.0071, +0.0273]$ & 1/7 \\
highlight-aware term (5 $-$ 11) & $-0.0023$ $[-0.0081, +0.0017]$ & 3/7 & $\mathbf{+0.0048}$$^{\star}$ $[+0.0025, +0.0071]$ & 16/58 & $\mathbf{-0.0342}$$^{\star}$ $[-0.0423, -0.0257]$ & 7/7 \\
\midrule
all three components (5 $-$ 3) & $-0.0005$ $[-0.0012, +0.0004]$ & 5/7 & $\mathbf{+0.0043}$$^{\star}$ $[+0.0026, +0.0061]$ & 16/58 & $\mathbf{-0.0250}$$^{\star}$ $[-0.0314, -0.0190]$ & 7/7 \\
\bottomrule
\end{tabular}
\end{table*}
```

**SCARED only**, single-column width, with the exact Wilcoxon $p$ that the $n=7$ datasets allow.

```latex
\begin{table}[t]
\centering
\caption{Contribution of each component of MonoIIF on SCARED, isolated. Each row is the paired difference in Abs Rel per test sequence ($n=7$) between the two configurations of Table~\ref{tab:components} that differ only in that component (first $-$ second, so a \textbf{negative} value means the component helps), with its 95\% cluster bootstrap interval, the number of sequences in which the first is better, and the exact two-sided Wilcoxon signed-rank $p$, whose smallest attainable value at $n=7$ is $2/2^{7}=0.016$. Bold marks intervals excluding zero. Line~4 is the full method on a randomly initialised encoder, so the first row measures the pretrained backbone with every other component held fixed.}
\label{tab:components-isolated-scared}
\small
\begin{tabular}{lcccc}
\toprule
Component isolated & $\Delta$ Abs Rel & 95\% CI & $k/n$ & exact $p$ \\
\midrule
pretrained weights (5 $-$ 4, everything else equal) & $\mathbf{-0.0549}$ & $[-0.0696, -0.0409]$ & 7/7 & 0.016 \\
DV-LoRA adapters (3 $-$ 2) & $\mathbf{-0.0089}$ & $[-0.0140, -0.0041]$ & 6/7 & 0.031 \\
DV-LoRA vs plain LoRA (5 $-$ 13) & $+0.0008$ & $[-0.0001, +0.0017]$ & 2/7 & 0.156 \\
local calibration alone (6 $-$ 3) & $+0.0000$ & $[-0.0018, +0.0022]$ & 3/7 & 1.000 \\
global calibration alone (7 $-$ 3) & $-0.0012$ & $[-0.0028, +0.0002]$ & 6/7 & 0.219 \\
illumination-invariant loss (5 $-$ 8) & $+0.0006$ & $[-0.0009, +0.0021]$ & 3/7 & 0.469 \\
highlight-aware term (5 $-$ 11) & $-0.0023$ & $[-0.0081, +0.0017]$ & 3/7 & 0.938 \\
\midrule
all three components (5 $-$ 3) & $-0.0005$ & $[-0.0013, +0.0003]$ & 5/7 & 0.375 \\
\bottomrule
\end{tabular}
\end{table}
```
